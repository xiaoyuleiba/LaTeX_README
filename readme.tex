\special{pdf:encrypt ownerpw (1234) length 128 perm 4}
\documentclass[a4paper,titlepage]{ctexart}
%\documentclass[a4paper,titlepage,xelatex]{bxjsarticle}
%\usepackage{xeCJK}
...
\begin{spacing}{1}
	...
	\begin{document}	%此为文章内容开始之标志
		\maketitle %输出标题
		\tableofcontents %输出目录
		\setcounter{page}{0} 将此页 (目录页) 的页码设为 0
		\clearpage %新开一页
		
		~~~~~~~~~~~~~~~~~~~~~~~~~~~(正文)~~~~~~~~~~~~~~~~~~~~~~~~~
		
		\section[在目录里显示的节名]{节名}
		
		~~~~~~~~~~~~~~~~~~~~~~~~~~~(正文)~~~~~~~~~~~~~~~~~~~~~~~~~
		
		\subsection[在目录里显示的小节名]{小节名}
		
		~~~~~~~~~~~~~~~~~~~~~~~~~~~(正文)~~~~~~~~~~~~~~~~~~~~~~~~~
		
		\subsubsection[在目录里显示的子小节名]{子小节名}
		
		~~~~~~~~~~~~~~~~~~~~~~~~~~~(正文)~~~~~~~~~~~~~~~~~~~~~~~~~
		
		\clearpage
		\bibliographystyle{unsrt}	%标明参考文献的样式
		\bibliography{reference.bib}	%引入参考文献, 大括号内为文献表位置
		
		\clearpage	%新起一页
		\appendixpage	%生成附录页
		\noappendicestocpagenum	%命令目录中的“附录”字样后方不标页码
...
\subsection{特殊命令\textasciitilde 导言区}
\par
...
\begin{enumerate}	
	\item 顶级命令区:
	\begin{enumerate}
		\item{加密pdf的特殊命令:} perm后的数字标识了开放的权限, 4为仅允许低质量打印: 点击打印按钮, 如果输出为一个新pdf会发现全部页面都被转化成了低分辨率图片; 2052为仅允许普通打印;此处省略了userpw项, 即打开文档时的密码. 须注意: {\Huge Lua\LaTeX 对此加密命令的兼容性貌似还不好.}
	...
	\begin{enumerate}
		...
		\begin{enumerate}		
			...
			\item 设置\verb|backend=biber|: 使之选定参考文献管理引擎为\verb|biber|. 选用\verb|biber|的原因是它在完全兼容\verb|bibtex|的基础上提供了更多的文献出处类型以及自定义选项;
			\item 设置\verb|url=true|: 有些\verb|.bib|代码会附上超链接, 可以按照不同需求选择表示与否;
			\item 设置\verb|doi=true|: 类似于超链接, 可选择表示\verb|DOI|号
			\item 设置\verb|dateabbrev=true|: 出版日期中的月份是否简写, 类似于\verb|November|简写为\verb|Nov.|;
			\item 设置\verb|backref=true|: 可选择在参考文献条目后表示此文献在哪几页被引用了, 并将此处的页码做成超链接;
			\item 设置\verb|style=numeric-comp|: ,maxnames=15|
		\end{enumerate}
	\end{enumerate}
...
\end{enumerate}
	...
	\item 抄写\label{verb}
		...
		\end{itemize}
	\item 列表
		\subitem 列表环境分为带序号与否2种: \verb|itemize|(无序)和\verb|enumerate|(有序).
		\subitem 两种列表环境可相互混合嵌套: 每一种列表环境最大可以达到4层\footnote{4层itemize加4层enumerate后, 就不能再向其中套任何新的这两种环境了. 实际上只允许套到总计第6层, 但是套到第7和第8也只是报一个非致命错误, 输出文件没问题}, 因为只做了4种不同层级的标识符.
...
\end{center}
\clearpage
\section{列表环境的层级与标识符示例}


\begin{itemize}
	\item  第1层\verb|itemize|
	\subitem subitem样式 (itemize-subitem) 
		\subsubitem subsubitem样式 (itemize-subitem-subsubitem) 
		\begin{itemize}
		\item 第2层\verb|itemize|
		\subitem subitem样式 (itemize $\times 2 +$ subitem) 
		\subsubitem subsubitem样式 (itemize $\times 2 +$ subitem-subsubitem) 
			\begin{itemize}
			\item 第3层\verb|itemize|
			\subitem subitem样式 (itemize $\times 3 +$ subitem) 
			\subsubitem subsubitem样式 (itemize $\times 3 +$ subitem-subsubitem) 
				\begin{itemize}
				\item 第4层\verb|itemize|
				\subitem subitem样式 (itemize $\times 4 +$ subitem) 
				\subsubitem subsubitem样式 (itemize $\times 4 +$ subitem-subsubitem) 
					\begin{enumerate}
						\item 第1层\verb|enumerate|
						\subitem subitem样式 (itemize $\times 4 +$ enumerate $+$ subitem) 
						\subsubitem subsubitem样式 (itemize $\times 4 +$ enumerate $+$ subitem-subsubitem) 
							\begin{enumerate}
								\item 第2层\verb|enumerate|
								\subitem subitem样式 (itemize $\times 4 +$ enumerate $\times 2 +$ subitem) 
								\subsubitem subsubitem样式 (itemize $\times 4 +$ enumerate $\times 2 +$ subitem-subsubitem) 
									\begin{enumerate}
										\item 第3层\verb|enumerate|
										\subitem subitem样式 (itemize $\times 4 +$ enumerate $\times 3 +$ subitem) 
										\subsubitem subsubitem样式 (itemize $\times 4 +$ enumerate $\times 3 +$ subitem-subsubitem) 
     											\begin{enumerate}
												\item 第4层\verb|enumerate|
										\subitem subitem样式 (itemize $\times 4 +$ enumerate $\times 4 +$ subitem) 
										\subsubitem subsubitem样式 (itemize $\times 4 +$ enumerate $\times 4 +$ subitem-subsubitem) 
										
												其实能输出这么多层itemize和enumerate, 但是第7层和第8层 (带begin和end的) 都会报错: 可能是学的html的h1 \textasciitilde  h6. 但是并不会不输出, 只会说 \verb|Too deeply nested.|
												
												而且, 里面还可以疯狂套subitem和subsubitem. 
												
												但是, 须注意, subitem和subsubitem行首是没有标识符的, 想要标识符就只能用begin和end新开一层. 正常人也开不了这么多层, 这里只是示范极限.
											\end{enumerate}
									\end{enumerate}
							\end{enumerate}
					\end{enumerate}
