...
\begin{spacing}{1}
	...
	\begin{document}	%此为文章内容开始之标志
		...
		\clearpage	%新起一页
		\appendixpage	%生成付録页
		\noappendicestocpagenum	%命令目录中的“付録”字样后方不标
		页码
		\addappheadtotoc	%将“付録”字样加入目录中
		\appendix	%规定此后为付録环境
		\section{付録标题}
		~~~~~~~~~~~~~~~~~~~~~~~~~(付録正文)~~~~~~~~~~~~~~~~~~~~~~~
...
\begin{enumerate}	
	...
	\begin{enumerate}
		...
		\item 调用付録样式的宏包\verb|appendix|;\label{付録导言区}
		\begin{itemize}
			\item 基于此宏包, 规定目录中出现“付録"字样, 默认值为不出现, 设为出现时则是出现“Appendices".
			\item 基于此宏包, 规定付録第一页左上角出现“付録"字样, 默认值为不出现, 设为出现时则是出现“Appendices".
		\end{itemize}
...
\item 超链接
	...
	\subitem 注意到: 
	\begin{itemize}
	\item \verb|\hyperref[落点名]{呈现的元素}|命令中的\verb|呈现的元素|可以是整整一章, 也可以是一整张图, 也可以是一张子图, 也可以是表(具体效果详见付録);
...
\item  付録
	\subitem 付録的设置同时需要在\hyperref[付録导言区]{导言区}和付録的代码部分前进行命令.
	\subitem 付録代码处需要用以下命令环绕
	\begin{spacing}{1}
	\begin{verbatim}
		\begin{appendices}
			设定
...
\addappheadtotoc    
	...
	\subitem 其作用分别为:
		\begin{enumerate}	
		\item 令付録章节名样式变为“付録 A”, “付録 B”\dots;
		\item 令付録第一页的左上角出现标识字样, 字样设定则是在导言区;
		\item 上一条命令定义的标识字样其实也被算作一种有序章节, 故而会在目录中出现页号, 此命令会抹去其页号;
		\item 令“付録”字样出现在目录中.
		\end{enumerate}
	\subitem 易知: \verb|\def\thesection{付録\Alph{section}}|可用于自定义各种元素的序号样式: \label{def}
		\begin{itemize}
			...
			\item 后面的大括号即为样式设定:
				...
				\end{verbatim}\label{付録文字}
				则之后的付録章节名会变成``\textit{电子烟假尼古丁真} a'', ``\textit{电子烟假尼古丁真} b'', \dots
				\subitem 各部分具体含义如下:
				\begin{enumerate}
					...
					\item 大括号内即为指定的样式:
						\begin{itemize}
							...
							\subitem 付録默认的\verb|\Alph|表示大写英文字母;
							...
							\subsubitem  当且仅当序号始终是不大于26且不小于1的整数时, 才能选择英文字母用作序号, 否则严重报错; 换言之, 付録, 第二层与第四层\verb|enumerate|环境内的条目数不得大于26个.
						\end{itemize}
				\end{enumerate}
		\end{itemize}
...
\item 参考文献
	...
	\subitem 参考文献的引用与参考文献列表的引入由以下设定实现:
		...
		\item 到期刊官网上, 往往能在引用界面看到一个\verb|BibTeX|的功能, 点击后会生成一段代码或是下载一个\verb|.bib|文件, 亦或是\verb|.ris|文件. 这就是\LaTeX 的参考文献数据文件. 
			...
			\item 在想生成参考文献列表的地方\footnote{习惯上, 参考文献列表一般在正文和付録之间, 是否要新起一页则是看具体情况下的要求}插入如下两行代码即可:
			\begin{verbatim}
...
\begin{itemize}
	...
	\item 日文使用\verb|\documentclass[a4paper,titlepage,uplatex,dvipdfmx]{jsarticle}|, 其中:
		...
		\end{enumerate}
		\subitem js系列中自带\hyperref[def]{此处}的命令, 故应将其删去. 另外, \hyperref[付録导言区]{导言区的字样设置}也应更改为日语汉字. 
